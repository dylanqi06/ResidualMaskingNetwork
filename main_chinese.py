...
def main(config_path):
    ...
    from trainers.chinese_trainer import ChineseTrainer

    # from trainers.centerloss_trainer import FER2013Trainer
    trainer = ChineseTrainer(model, train_set, val_set, test_set, configs)
...
def get_dataset(configs):
    ...
    """
    from utils.datasets.chinese import chinese2023

    # todo: add transform
    train_set = chinese2023("train", configs)
    val_set = chinese2023("val", configs)
    test_set = chinese2023("test", configs, tta=True, tta_size=10)
    return train_set, val_set, test_set
    # now we are at stage, need to decide the dataset

if __name__ == "__main__":
    main("./configs/chinese_config.json")
